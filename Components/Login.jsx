    ...
    <>
     ...
     <div class="flex items-center relative min-h-screen p-4 bg-gray-100 lg:justify-center">
      ...
      >
        ...
        >
          <div class="my-3 text-2xl font-bold tracking-wider text-center">
        ...
        <div class="p-5 bg-white md:flex-1">
          <h3 class="my-4 text-2xl font-semibold text-gray-700">Login With Email</h3>
          <form action="#" class="flex flex-col space-y-5">
            <div class="flex flex-col space-y-1">
              {message && <p>{message}</p>}
      {!otpSent ? (
        <div>
          <input className="w-full px-4 py-2 mb-4 transition duration-300 border border-gray-300 rounded focus:border-transparent focus:outline-none focus:ring-4 focus:ring-blue-200" type="email" value={email} onChange={handleEmailChange} placeholder="Enter your email" />
          <button className="w-full px-4 py-2 text-lg font-semibold text-white transition-colors duration-300 bg-blue-500 rounded-md shadow hover:bg-blue-600 focus:outline-none focus:ring-blue-200 focus:ring-4" onClick={handleSendOtp}>Send OTP</button>
        </div>
      ) : (
        <div>
          <input className="px-4 py-2 mb-4 transition duration-300 border border-gray-300 rounded focus:border-transparent focus:outline-none focus:ring-4 focus:ring-blue-200" type="text" value={otp} onChange={handleOtpChange} placeholder="Enter OTP" />
          <button className="w-full px-4 py-2 text-lg font-semibold text-white transition-colors duration-300 bg-blue-500 rounded-md shadow hover:bg-blue-600 focus:outline-none focus:ring-blue-200 focus:ring-4" onClick={handleVerifyOtp}>Verify OTP</button>
        </div>
      )}
            </div>
